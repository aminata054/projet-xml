...
<head>
    ...
    .chat-container {
        width: 700px;
        /* Augmentation de la largeur pour un carré plus grand */
        height: 700px;
        /* Hauteur ajustée pour égaler la largeur, créant un carré */
        overflow-y: auto;
...
<body>
    <div class="container chat-container">
        ...
        <form method="post" action="add.php" enctype="multipart/form-data" class="profile-form" id="create-group-form">
            <div class="form-group">
                ...
                <input type="text" name="nom_groupe" id="nom_groupe" class="form-input" required maxlength="50"
                    placeholder="Ex: Projet XML">
    ...
    function removeMemberFromSelected(contactId) {
        const memberDiv = document.querySelector(
            `#selected-members-list .selected-member[data-contact-id="${contactId}"]`);
        if (memberDiv) {
            memberDiv.remove();
        }
    ...
    function removeMember(contactId) {
        const contactItem = document.querySelector(
            `#available-contacts-list .contact-item[data-contact-id="${contactId}"]`);
        if (contactItem) {
